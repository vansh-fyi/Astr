...
final List<({List<int> rgb, int bortle})> ZONE_COLORS = <({int bortle, List<int> rgb})>[
  (rgb: <int>[0, 0, 50], bortle: 1),
  (rgb: <int>[0, 24, 73], bortle: 1),
  (rgb: <int>[0, 61, 102], bortle: 2),
  (rgb: <int>[0, 85, 127], bortle: 2),
  (rgb: <int>[0, 110, 51], bortle: 3),
  (rgb: <int>[61, 153, 61], bortle: 4),
  (rgb: <int>[153, 153, 0], bortle: 4),
  (rgb: <int>[204, 204, 0], bortle: 5),
  (rgb: <int>[204, 102, 0], bortle: 5),
  (rgb: <int>[255, 153, 51], bortle: 6),
  (rgb: <int>[204, 0, 0], bortle: 7),
  (rgb: <int>[255, 51, 51], bortle: 8),
  (rgb: <int>[255, 153, 153], bortle: 8),
  (rgb: <int>[255, 204, 204], bortle: 9),
  (rgb: <int>[255, 255, 255], bortle: 9),
];
...
Future<void> main() async {
  ...
  final Directory tempDir = Directory('temp_cities_data');
  ...
  try {
    ...
    final String zipPath = '${tempDir.path}/cities.zip';
    final String txtPath = '${tempDir.path}/cities15000.txt';

    // 1. Download Zip
    final ProcessResult downloadResult = await Process.run('curl', <String>['-L', '-o', zipPath, GEONAMES_URL]);
    if (downloadResult.exitCode != 0) {
      throw Exception('Download failed: ${downloadResult.stderr}');
    }

    // 2. Unzip
    final ProcessResult unzipResult = await Process.run('unzip', <String>['-o', zipPath, '-d', tempDir.path]);
    if (unzipResult.exitCode != 0) {
      throw Exception('Unzip failed: ${unzipResult.stderr}');
    }

    print('Step 2: Parsing GeoNames data...');
    final Stream<List<int>> input = File(txtPath).openRead();
    final List<String> lines = await input.transform(utf8.decoder).transform(const LineSplitter()).toList();
    ...
    for (int i = 0; i < lines.length; i++) {
        ...
        try {
            ...
            final double lat = double.parse(parts[4]);
            final double lng = double.parse(parts[5]);
            final String popStr = parts[14];
            final double pop = popStr.isNotEmpty ? double.parse(popStr) : 0.0;
    ...
    cities.addAll(<City>[
        ...
        City(name: 'Rural Test Point', lat: 25, lng: 10, pop: 20000), // Sahara
    ]);
  }

  // Phase 2: Processing & Map Lookup
  try {
    cities.sort((City a, City b) => b.pop.compareTo(a.pop));
    final List<City> selectedCities = cities.take(MAX_ENTRIES).toList();
    print('Selected top ${selectedCities.length} cities.');

    print('Step 3: Calculating Bortle Class from Map...');
    final File mapFile = File(MAP_PATH);
    if (!mapFile.existsSync()) throw Exception('Map file missing: $MAP_PATH');
    
    final img.Image? image = img.decodeImage(await mapFile.readAsBytes());
    if (image == null) throw Exception('Failed to decode map image');

    final List processedData = <dynamic>[];

    for (final City city in selectedCities) {
        final int x = ((city.lng + 180) * (image.width / 360)).toInt();
        final int y = ((90 - city.lat) * (image.height / 180)).toInt();
    ...
    final File outFile = File(OUTPUT_PATH);
...
int getBortleFromColor(img.Pixel pixel) {
    ...
    int bestBortle = 5;
    for (final ({int bortle, List<int> rgb}) zone in ZONE_COLORS) {
        final double dist = sqrt(pow(r - zone.rgb[0], 2) + pow(g - zone.rgb[1], 2) + pow(b - zone.rgb[2], 2));
