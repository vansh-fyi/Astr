...
@GenerateNiceMocks([MockSpec<Canvas>()])
import 'visibility_graph_painter_test.mocks.dart';

void main() {
  final startTime = DateTime(2025, 12, 3, 18, 0); // 6 PM
  final endTime = DateTime(2025, 12, 4, 6, 0); // 6 AM (12 hours)
  ...
  final data = VisibilityGraphData(
  ...
  test('VisibilityGraphPainter draws current position indicator at correct coordinates', () {
    ...
    final size = const Size(400, 200); // 400px width, 200px height
    ...
    final currentTime = startTime.add(const Duration(hours: 3));

    final painter = VisibilityGraphPainter(
      data: data,
      scrubberPosition: -1,
      startTime: startTime,
      endTime: endTime,
  ...
  test('VisibilityGraphPainter uses correct highlight color', () {
    ...
    final paintVerification = verify(canvas.drawCircle(
      any, 
      5, 
      captureAny,
    ));
    ...
    final paints = paintVerification.captured.cast<Paint>();
    expect(paints.length, 3);

    // Debug: Print styles and colors if needed (can't see console easily, so rely on logic)
    
    // Find Stroke (Blue)
    final strokePaint = paints.firstWhere((p) => 
      p.style == PaintingStyle.stroke && 
      p.color.value == Colors.blue.value,
      orElse: () => throw Exception('Stroke paint not found. Paints: ${paints.map((p) => '${p.style}, ${p.color}').toList()}'),
    );
    
    // Find Fill (White)
    final fillPaint = paints.firstWhere((p) => 
    ...
    final glowPaint = paints.firstWhere((p) => 
  ...
  test('VisibilityGraphPainter does NOT draw indicator if time is out of range', () {
    ...
    // Time before start
    final currentTime = startTime.subtract(const Duration(hours: 1));
