...
void main() {
  ...
  group('DatabaseService', () {
    ...
    test('getDatabase returns initialized database', () async {
      ...
      final Result dbResult = await service.getDatabase();
    ...
    test('query executes successfully on stars table', () async {
      ...
      final Result<List<Map<String, dynamic>>> result = await service.query('stars', limit: 10);
    ...
    test('query with where clause works correctly', () async {
      ...
      final Result<List<Map<String, dynamic>>> result = await service.query(
        'stars',
        where: 'name = ?',
        whereArgs: <Object?>['Sirius'],
      );

      expect(result.isSuccess, true);
      expect(result.value.length, greaterThan(0));

      final Map<String, dynamic> star = result.value.first;
    ...
    test('rawQuery executes successfully', () async {
      ...
      final Result<List<Map<String, dynamic>>> result = await service.rawQuery(
    ...
    test('multiple initialize calls are safe', () async {
      final Result<void> result1 = await service.initialize();
      final Result<void> result2 = await service.initialize();
