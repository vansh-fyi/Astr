...
class _AltitudeGraphState extends State<AltitudeGraph> {
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        return GestureDetector(
          onHorizontalDragStart: (DragStartDetails details) => _updateScrubber(details.localPosition.dx, constraints.maxWidth),
          onHorizontalDragUpdate: (DragUpdateDetails details) => _updateScrubber(details.localPosition.dx, constraints.maxWidth),
          onHorizontalDragEnd: (_) => _resetScrubber(),
          onTapDown: (TapDownDetails details) => _updateScrubber(details.localPosition.dx, constraints.maxWidth),
  ...
  void _updateScrubber(double localDx, double width) {
    final double position = (localDx / width).clamp(0.0, 1.0);
    ...
    final double currentHour = (startHour + (position * totalHours)) % 24;
    final int minute = ((position * totalHours * 60) % 60).round();
    
    final String timeLabel = '${currentHour.floor().toString().padLeft(2, '0')}:${minute.toString().padLeft(2, '0')}';
    
    // Calculate Altitude (Sine wave approximation)
    final double yNormalized = sin(position * pi);
    final double altitude = yNormalized * 80; // Max 80 degrees
...
class _AltitudeGraphPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double x = 0; x <= width; x++) {
      ...
      final double yNormalized = sin(t * pi); // 0 -> 1 -> 0
      final double y = height - (yNormalized * height * 0.8);
    ...
    // Draw Peak Dot
    final double peakX = width * 0.5;
    final double peakY = height - (height * 0.8);
    canvas.drawCircle(Offset(peakX, peakY), 5, _peakDotPaint);

    // Draw Current Time Indicator (Static "Now")
    // Let's say "Now" is at 0.2
    final double nowX = width * 0.2;
    const double nowT = 0.2;
    final double nowYNormalized = sin(nowT * pi);
    final double nowY = height - (nowYNormalized * height * 0.8);
    ...
    if (scrubberPosition >= 0) {
      final double x = scrubberPosition * width;
      canvas.drawLine(
        Offset(x, 0),
        Offset(x, height),
        _scrubberLinePaint,
      ...
      final double scrubYNormalized = sin(scrubberPosition * pi);
      final double scrubY = height - (scrubYNormalized * height * 0.8);
